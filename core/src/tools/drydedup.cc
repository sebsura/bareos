...
bool digest_message(const char *message,
		    size_t message_len,
		    char **digest,
		    unsigned int *digest_len)
{
	...
	if((*digest = (char *)OPENSSL_malloc(EVP_MD_size(EVP_sha256()))) == NULL)
		return false;
...
struct block {
	...
	char fill) : data(size, fill)
  {
    char *s;
    unsigned int length;
    if (!digest_message(&*begin, end-begin, &s, &length)) {
      throw;
    }
    checksum = std::string(s, length);
