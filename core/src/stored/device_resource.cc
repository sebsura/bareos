...
{
  auto device = std::make_unique<DeviceResource>();

  if (media_type) { device->media_type = strdup(media_type); }
  if (archive_device_string) {
    device->archive_device_string = strdup(archive_device_string);
  }
  if (device_options) { device->device_options = strdup(device_options); }
  if (diag_device_name) { device->diag_device_name = strdup(diag_device_name); }
  if (changer_name) { device->changer_name = strdup(changer_name); }
  if (changer_command) { device->changer_command = strdup(changer_command); }
  if (alert_command) { device->alert_command = strdup(alert_command); }
  if (spool_directory) { device->spool_directory = strdup(spool_directory); }
  device->device_type = device_type;
  device->label_type = label_type;
  device->access_mode = access_mode;
  device->autoselect = autoselect;
  device->norewindonclose = norewindonclose;
  device->drive_tapealert_enabled = drive_tapealert_enabled;
  device->drive_crypto_enabled = drive_crypto_enabled;
  device->query_crypto_status = query_crypto_status;
  device->collectstats = collectstats;
  device->eof_on_error_is_eot = eof_on_error_is_eot;
  device->drive = drive;
  device->drive_index = drive_index;
  memcpy(device->cap_bits, cap_bits, CAP_BYTES);
  device->max_changer_wait = max_changer_wait;
  device->max_rewind_wait = max_rewind_wait;
  device->max_open_wait = max_open_wait;
  device->max_open_vols = max_open_vols;
  device->label_block_size = label_block_size;
  device->min_block_size = min_block_size;
  device->max_block_size = max_block_size;
  device->max_network_buffer_size = max_network_buffer_size;
  device->max_concurrent_jobs = max_concurrent_jobs;
  device->autodeflate_algorithm = autodeflate_algorithm;
  device->autodeflate_level = autodeflate_level;
  device->autodeflate = autodeflate;
  device->autoinflate = autoinflate;
  device->vol_poll_interval = vol_poll_interval;
  device->max_file_size = max_file_size;
  device->volume_capacity = volume_capacity;
  device->max_spool_size = max_spool_size;
  device->max_job_spool_size = max_job_spool_size;

  if (mount_point) { device->mount_point = strdup(mount_point); }
  if (mount_command) { device->mount_command = strdup(mount_command); }
  if (unmount_command) { device->unmount_command = strdup(unmount_command); }
  device->dev = dev;
  device->changer_res = changer_res;


  device->description_ = strdup(description_);
  device->rcode_ = rcode_;
  device->refcnt_ = refcnt_;
  device->rcode_str_ = rcode_str_;
  device->item_present_ = item_present_;

  memcpy(device->inherit_content_, inherit_content_,
         sizeof(device->inherit_content_));
  device->internal_ = internal_;

  device->resource_name_ = strndup(copy_name.data(), copy_name.size());
