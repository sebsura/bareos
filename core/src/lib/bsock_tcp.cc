...
#ifdef WIN_DUPLICATE_SOCKET
static int DuplicateSocket(int sock)
{
  SOCKET insock = static_cast<SOCKET>(sock);

  WSAPROTOCOL_INFOA info;
  if (!WSADuplicateSocketA(insock, ::GetCurrentProcessId(), &info)) {
    return -1;
  }

  SOCKET outsock = WSASocketA(0, 0, 0, &info, 0, 0);

  return static_cast<int>(outsock);
}
#endif
...
#ifdef WIN_DUPLICATE_SOCKET
  if (fd_ >= 0) { clone->fd_ = DuplicateSocket(fd_); }
  if (spool_fd_ >= 0) { clone->spool_fd_ = DuplicateSocket(spool_fd_); }
#else
 ...
 * this routine.
 */
bool BareosSocketTCP::connect(JobControlRecord* jcr,
                              int retry_interval,
...
bool BareosSocketTCP::send()
{
  ...
  if (IsTerminated()) {
    if (!suppress_error_msgs_) {
      Qmsg4(jcr_, M_ERROR, 0,
            _("Socket is terminated=%d on call to %s:%s:%d\n"), IsTerminated(),
            who_, host_, port_);
    }
    return false;
  }
