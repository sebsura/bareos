...
class PoolMem {
 ...
 public:
  ...
  int bsprintf(const char* fmt, ...) PRINTF_LIKE(2, 3);
  int Bvsprintf(const char* fmt, va_list arg_ptr) PRINTF_LIKE(2, 0);
};
...
int PmFormat(POOLMEM*& dest_pm, const char* fmt, ...) PRINTF_LIKE(2, 3);
