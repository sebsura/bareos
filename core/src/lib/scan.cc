...
std::string_view StripTrailingJunk(std::string_view cmd)
{
  ...
  if (pos != cmd.npos) { cmd.remove_suffix(cmd.size() - pos - 1); }
...
int fstrsch(const char* a, const char* b) /* folded case search */
{
  ...
  while (*a) { /* do it over the correct slow way */
    if (B_ISUPPER(c1 = *a)) { c1 = tolower((int)c1); }
    if (B_ISUPPER(c2 = *b)) { c2 = tolower((int)c2); }
    if (c1 != c2) { return 0; }
    a++;
    b++;
  }
              ...
              int max_args)
{
  ...
  for (int i = 0; i < *argc; i++) {
    p = strchr(argk[i], '=');
    if (p) { *p++ = 0; /* Terminate keyword and point to value */ }
  ...
  while (*fmt && !error) {
    ...
    switch_top:
      switch (*fmt++) {
        ...
        case 'h':
          h = 1;
          if (*fmt == 'd' || *fmt == 'u') { goto switch_top; }
          error = true;
          break;
        case 'l':
          l = 1;
