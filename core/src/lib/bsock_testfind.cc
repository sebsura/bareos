...
BareosSocketTestfind::BareosSocketTestfind() : BareosSocket() {}

BareosSocketTestfind::~BareosSocketTestfind() { destroy(); }

BareosSocket* BareosSocketTestfind::clone()
{
  BareosSocketTestfind* clone = new BareosSocketTestfind(*this);
 ...
 * this routine.
 */
bool BareosSocketTestfind::connect(JobControlRecord* jcr,
                                   int retry_interval,
                                   ...
                                   bool verbose)
{
  ...
  for (i = 0; !open(jcr, name, host, service, port, heart_beat, &fatal);
       i -= retry_interval) {
    BErrNo be;
    if (fatal || (jcr && JobCanceled(jcr))) { goto bail_out; }
    ...
    if (i < 0) {
      ...
      if (verbose)
        Qmsg4(jcr, M_WARNING, 0,
              _("Could not connect to %s on %s:%d. ERR=%s\n"
                "Retrying ...\n"),
...
bail_out:
  if (tid) { StopThreadTimer(tid); }
  return ok;
}

// Finish initialization of the pocket structure.
void BareosSocketTestfind::FinInit(JobControlRecord* jcr,
                                   int sockfd,
                                   const char* who,
                                   const char* host,
                                   int port,
 ...
 */
bool BareosSocketTestfind::open(JobControlRecord* jcr,
                                const char* name,
                                const char* host,
                                char* service,
                                int port,
                                ...
                                int* fatal)
{
  ...
  if ((addr_list = BnetHost2IpAddrs(host, 0, &errstr)) == NULL) {
    ...
    Qmsg2(jcr, M_ERROR, 0,
          _("BnetHost2IpAddrs() for host \"%s\" failed: ERR=%s\n"), host,
          errstr);
        ...
        default:
          *fatal = 1;
          Pmsg3(000, _("Socket open error. proto=%d port=%d. ERR=%s\n"),
                ipaddr->GetFamily(), ipaddr->GetPortHostOrder(),
                be.bstrerror());
          break;
      }
      continue;
...
bool BareosSocketTestfind::SetKeepalive(JobControlRecord* jcr,
                                        int sockfd,
                                        bool enable,
                                        int keepalive_start,
...
bool BareosSocketTestfind::SendPacket(int32_t* hdr, int32_t pktsiz)
{
  Enter(400);
 ...
 */
int32_t BareosSocketTestfind::recv()
{
  int32_t nbytes;
  int32_t pktsiz;
...
int BareosSocketTestfind::GetPeer(char* buf, socklen_t buflen)
{
 ...
 *          true  on success
 */
bool BareosSocketTestfind::SetBufferSize(uint32_t size, int rw)
{
  ...
  if ((msg = ReallocPoolMemory(msg, dbuf_size + 100)) == NULL) {
    Qmsg0(get_jcr(), M_FATAL, 0,
          _("Could not malloc BareosSocket data buffer\n"));
    return false;
  }
 ...
 */
int BareosSocketTestfind::SetNonblocking()
{
#ifndef HAVE_WIN32
  int oflags;
 ...
 */
int BareosSocketTestfind::SetBlocking()
{
#ifndef HAVE_WIN32
  int oflags;
...
void BareosSocketTestfind::RestoreBlocking(int flags)
{
#ifndef HAVE_WIN32
 ...
 *         -1 if error
 */
int BareosSocketTestfind::WaitData(int sec, int usec)
{
  int msec;
...
int BareosSocketTestfind::WaitDataIntr(int sec, int usec)
{
  int msec;
...
void BareosSocketTestfind::close()
{
  /* if not cloned */
  ClearLocking();
  ...
  if (fd_ >= 0) {
    if (!cloned_) {
      if (IsTimedOut()) { shutdown(fd_, SHUT_RDWR); }
    }
    socketClose(fd_);
    fd_ = -1;
  }
...
void BareosSocketTestfind::destroy()
{
 ...
 * read requests
 */
int32_t BareosSocketTestfind::read_nbytes(char* ptr, int32_t nbytes)
{
  int32_t nleft, nread;
 ...
 */
int32_t BareosSocketTestfind::write_nbytes(char* ptr, int32_t nbytes)
{
  ...
  while (nleft > 0) {
    ...
    nleft -= nwritten;
    ptr += nwritten;
    if (UseBwlimit()) { ControlBwlimit(nwritten); }
  }

  return nbytes - nleft;
}

bool BareosSocketTestfind::ConnectionReceivedTerminateSignal()
{
  int32_t signal;
  SetNonblocking();
  bool terminated = false;
  if (::recv(fd_, (char*)&signal, 4, MSG_PEEK) == 4) {
    signal = ntohl(signal);
