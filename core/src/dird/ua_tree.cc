...
bool UserSelectFilesFromTree(TreeContext* tree)
{
  ...
  if (!cwd.empty()) { ua->SendMsg(T_("cwd is: %s\n"), cwd.c_str()); }
...
int InsertTreeHandler(void* ctx, int, char** row)
{
  ...
  if (ok) {
    ...
    if (tree->all) {
      ...
      if (type == TreeNodeType::Dir || type == TreeNodeType::DirNLS) {
                      ...
                      bool extract)
{
  ...
  if (node->type == TreeNodeType::Dir || node->type == TreeNodeType::DirNLS) {
...
static int HelpCmd(UaContext* ua, TreeContext*)
{
  ...
  for (i = 0; i < comsize; i++) {
    // List only non-dot commands
    if (restore_browser_commands[i].key[0] != '.') {
      ua->SendMsg("  %-10s %s\n", T_(restore_browser_commands[i].key),
                  T_(restore_browser_commands[i].help));
    }
  }
  ua->SendMsg("\n");
  return 1;
