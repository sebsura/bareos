...
inline constexpr const char sql_jobids_from_job[]
...
inline constexpr const char sql_client[]
...
inline constexpr const char sql_jobids_from_client[]
...
inline constexpr const char sql_vol[]
...
inline constexpr const char sql_jobids_from_vol[]
...
inline constexpr const char sql_smallest_vol[]
...
inline constexpr const char sql_oldest_vol[]
    = "SELECT Media.MediaId FROM Media,Pool,JobMedia WHERE"
      " Media.MediaId in (SELECT DISTINCT MediaId from JobMedia) AND"
      " Media.VolStatus in ('Full','Used','Error') AND Media.Enabled=1 AND"
      " Media.PoolId=Pool.PoolId AND Pool.Name='%s'"
...
inline constexpr const char sql_jobids_from_mediaid[]
...
inline constexpr const char sql_pool_bytes[]
...
inline constexpr const char sql_job_bytes[]
    = "SELECT SUM(JobBytes) FROM Job WHERE JobId IN (%s)";

// Get Media Ids in Pool
inline constexpr const char sql_mediaids[]
...
inline constexpr const char sql_pool_time[]
    = "SELECT DISTINCT Job.JobId FROM Pool,Job,Media,JobMedia WHERE"
      " Pool.Name='%s' AND Media.PoolId=Pool.PoolId AND"
      " VolStatus IN ('Full','Used','Error') AND Media.Enabled=1 AND"
      " Job.Type IN ('B','C') AND Job.JobStatus IN ('T','W') AND"
 ...
 * before
 */
inline constexpr const char sql_jobids_of_pool_uncopied_jobs[]
    = "SELECT DISTINCT Job.JobId,Job.StartTime FROM Job,Pool"
...
inline constexpr const char sql_migrate_ndmp_metadata[]
...
inline constexpr const char sql_copy_ndmp_metadata[]
...
inline constexpr int dbglevel = 10;
                          ...
                          const char* where)
{
  if (!next_pool) {
    Jmsg(jcr, M_FATAL, 0,
         T_("No Next Pool specification found in Pool \"%s\".\n"),
         pool->resource_name_);
    return false;
  }
  ...
  if (!next_pool->storage || next_pool->storage->size() == 0) {
    Jmsg(jcr, M_FATAL, 0,
         T_("No Storage specification found in Next Pool \"%s\".\n"),
                                        ...
                                        PoolResource** retpool)
{
  ...
  *retpool = pool;
  if (!pool) {
    Jmsg(jcr, M_FATAL, 0, T_("Pool resource \"%s\" not found.\n"), pr.Name);
    return false;
  }
...
static int UniqueDbidHandler(void* ctx, int, char** row)
{
  idpkt* ids = (idpkt*)ctx;
...
static int UniqueNameHandler(void* ctx, int, char** row)
{
                                      ...
                                      const char* type)
{
  ...
  if (ids->count == 0) {
    Jmsg(jcr, M_INFO, 0, T_("No %ss found to %s.\n"), type,
         jcr->get_ActionName());
  }
                                     ...
                                     const char* type)
{
  ...
  if (ids->count == 0) {
    Jmsg(jcr, M_INFO, 0, T_("No %s found to %s.\n"), type,
         jcr->get_ActionName());
                                                ...
                                                idpkt* ids)
{
  ...
  if (!jcr->is_JobType(JT_COPY)) {
    Jmsg(jcr, M_FATAL, 0,
         T_("Selection Type 'pooluncopiedjobs' only applies to Copy Jobs\n"));
    return false;
  }
                              ...
                              const char* type)
{
  ...
  if (!jcr->dir_impl->res.job->selection_pattern) {
    Jmsg(jcr, M_FATAL, 0, T_("No %s %s selection pattern specified.\n"),
  ...
  if (item_chain->size() == 0) {
    Jmsg(jcr, M_INFO, 0, T_("Query of Pool \"%s\" returned no Jobs to %s.\n"),
    ...
    if (rc != 0) {
      ...
      Jmsg(jcr, M_FATAL, 0,
           T_("Could not compile regex pattern \"%s\" ERR=%s\n"),
  ...
  if (item_chain->size() == 0) {
    Jmsg(jcr, M_INFO, 0, T_("Regex pattern matched no Jobs to %s.\n"),
         jcr->get_ActionName());
    ok = true;
    goto bail_out; /* skip regex match */
  }
...
static inline bool getJobs_to_migrate(JobControlRecord* jcr)
{
  ...
  switch (jcr->dir_impl->res.job->selection_type) {
    ...
    case MT_SQLQUERY:
      if (!jcr->dir_impl->res.job->selection_pattern) {
        Jmsg(jcr, M_FATAL, 0, T_("No %s SQL selection pattern specified.\n"),
             jcr->get_OperationName());
        goto bail_out;
      }
    ...
    case MT_POOL_OCCUPANCY: {
      ...
      if (ctx.count == 0) {
        Jmsg(jcr, M_INFO, 0, T_("No Volumes found to %s.\n"),
             jcr->get_ActionName());
        retval = true;
        goto bail_out;
      }
    ...
    case MT_POOL_UNCOPIED_JOBS:
      ...
      break;
    default:
      Jmsg(jcr, M_FATAL, 0, T_("Unknown %s Selection Type.\n"),
           jcr->get_OperationName());
      goto bail_out;
  }

  if (ids.count == 0) {
    Jmsg(jcr, M_INFO, 0, T_("No JobIds found to %s.\n"), jcr->get_ActionName());
    retval = true;
    goto bail_out;
  }

  Jmsg(jcr, M_INFO, 0, T_("The following %u JobId%s chosen to be %s: %s\n"),
  ...
  if (jcr->dir_impl->MigrateJobId != 0) {
    ...
    if (prev_jr.JobId == 0 || jcr->dir_impl->ExpectedFiles == 0) {
      ...
      if (prev_jr.JobId == 0) {
        ...
        Jmsg(jcr, M_INFO, 0, T_("Previous Job has no data to %s.\n"),
             jcr->get_ActionName());
      }
    ...
    if (!SetupJob(mig_jcr, true)) {
      Jmsg(jcr, M_FATAL, 0, T_("setup job failed.\n"));
      return false;
    }
    ...
    if (SetMigrationNextPool(jcr, &pool)) {
      ...
      CopyRstorage(mig_jcr, pool->storage, T_("Pool resource"));
      CopyRstorage(jcr, pool->storage, T_("Pool resource"));
    ...
    if (!jcr->dir_impl->res.read_storage_list) {
      CopyRstorage(jcr, prev_job->storage, T_("previous Job"));
    }
...
static inline bool DoActualMigration(JobControlRecord* jcr)
{
  ...
  Jmsg(jcr, M_INFO, 0, T_("Start %s JobId %s, Job=%s\n"),
