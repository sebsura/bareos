...
If the -f option is not specified, :command:`bsmtp` will use your userid. If the option -h is not specified :command:`bsmtp` will use the value in the environment variable SMTPSERVER or if there is none localhost. By default port 25 is used.
...
.. index::
   single: bdedupestimate
   single: Command; bdedupestimate

:command:`bdedupestimate` is a program that can estimate the deduplication
potential of your backups.

To run it, use:

.. literalinclude:: /include/autogenerated/usage/bdedupestimate.txt

The basic idea is that it will read all the data from the given volumes. It then
splits that data into blocks of the given blocksize and compares those blocks
between one another. Once finished it will then output both the input size as
well as the size necessary after deduplication.
For an optimal result you should ensure that the given blocksize divides the
network buffer size.

.. include:: ../man/bdedupestimate.rst
